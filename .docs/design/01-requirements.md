...
| Role | Method | Endpoint | 기능 | 상세 로직 및 제약사항 |
| :--- | :---: | :--- | :--- | :--- |
| User | `POST` | `/coupons/{id}/issue` | **쿠폰 발급** | 선착순/중복 방지 |
| User | `GET` | `/users/me/coupons` | **내 쿠폰 조회** | 사용 가능 여부 포함 |

#### 상세 요구사항
* **발급 제약**: 유저당 1회 제한. 전체 발행량 소진 시 발급 불가(`Sold Out`).
* **조회 정보**: 쿠폰명, 할인금액(율), 최소주문금액, 만료일, 상태(사용가능/사용완료/만료).
